      ...
      {projects.map((project, index) => (
        <article className={`portfolio-card ${project.accent}`} style={{ '--card-index': index }} key={project.id}>
          <div className="portfolio-preview">
            {project.url ? (
              <picture><source media="(max-width: 800px)" srcSet={project.mobileImage || project.image}/><img src={project.image} alt={`Aperçu du site ${project.title}`} loading="lazy" fetchPriority="low" decoding="async" width="1400" height="900" /></picture>
            ) : (
